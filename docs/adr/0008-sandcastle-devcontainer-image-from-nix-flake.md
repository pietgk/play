...
**`sh -c` vs login shells — the toolchain is on the default PATH, not only in login shells.** Baking the devshell into `/etc/profile.d` covers the agent (Codex runs `bash -lc`), but Sandcastle also issues its own internal commands — `git config --global --add safe.directory`, worktree ops — via bare `sh -c`, which sources no profile. With git coming only from the flake, those failed with `git: not found`. Fix: a shared `toolPackages` list in `flake.nix` feeds both the `devShell` and a `packages.dev` `buildEnv`, and the image `nix profile install`s `dev` so node/pnpm/git/gh land in `~/.nix-profile/bin` — already on the image's `ENV PATH`, hence resolvable by *every* shell. The login-shell devshell env (`PLAYWRIGHT_*`, `node_modules/.bin`) layers on top for the agent. One tool list, two surfaces (interactive devShell + always-on PATH), no drift.

**Credential copy — `auth.json` only, never the whole `~/.codex`.** The original `onSandboxReady` hook ran `cp -r /mnt/host-codex/. /home/agent/.codex/`, copying the *entire* host Codex home into the container. The Decision above (line: "`~/.codex` auth ... copied to the agent home") only ever intended the **credential**, but the wholesale copy dragged in 213MB of host-machine state: `sessions/`, logs, model caches, and — fatally — `state_5.sqlite`, whose rows map thread IDs to **host-absolute** rollout paths (`/Users/grop/.codex/sessions/…jsonl`). On a Linux container those paths don't exist, so Codex's startup `codex_rollout::list` returned a *stale rollout path* and exited 1 **before reading the prompt** — every run, until the host session db happened to align. `config.toml` was equally toxic: it pins `CODEX_HOME = "/Users/grop/.codex"` and host-only `notify`/plugin/marketplace/`[projects."/Users/…"]` paths. Fix: copy **only `auth.json`**. Codex runs with `--dangerously-bypass-approvals-and-sandbox` (so no project-trust config is needed) and regenerates a clean state db + session index per run. The container's Codex state is now reproducible and structurally immune to host-path staleness — the class of bug, not just this instance, is gone.
